...
function initEvents(block) {
  block.addEventListener('click', (ev) => {
    ...
    sections.forEach((section) => (section.dataset.tab === tab
      ? section.classList.remove('hidden')
      : section.classList.add('hidden')));
    buttons.forEach((button) => (button === ev.target
...
export default async function decorate(block) {
  ...
  tabSections.forEach((section, idx) => (idx === 0 ? section.classList.remove('hidden') : section.classList.add('hidden')));
